...
На Windows откройте папку проекта и запустите `START_MESM.bat`. Он проверит данные и экран кассового контура, затем покажет адрес `http://127.0.0.1:8501`. В разделе «Кассовый контур» по умолчанию открыт явно помеченный синтетический пример; CSV-шаблоны можно скачать на том же экране. Для проверки собственной выгрузки выключите демонстрационный набор, загрузите строки и при необходимости утверждённые корректировки, выберите единицы измерения и дату доступности. Файлы остаются в текущем сеансе; в репозиторий они не записываются.
